...
get_all_worker_cores(
    ...
    for (uint32_t full_send_direction = 0; full_send_direction < num_full_send_directions; ++full_send_direction) {
        ...
        for (uint32_t link = 0; link < num_links; ++link) {
            ...
            worker_core_map[std::make_pair(link, full_send_direction)] = {
                CoreRangeSet(local_receiver_worker_cores), CoreRangeSet(local_sender_worker_cores)};
        }
    }
            ...
            switch (topology) {
                ...
                case ccl::Topology::Ring:
                    switch (all_gather_config.get_bidirectional_mode()) {
                        ...
                        default:
                            TT_THROW(
                                "Unsupported bidirectional mode {}. Please change.",
...
tt::tt_metal::operation::ProgramWithCallbacks all_gather_multi_core_with_workers_helper(
    ...
    auto const& all_gather_config = AllGatherConfig(
        input_tensor,
        output_tensor,
        dim,
        ring_size,
    ...
    for (uint32_t link = 0; link < num_links; link++) {
        edm_sem_addrs_per_link.at(link).reserve(
            all_gather_config.get_num_workers_per_link() * num_full_send_directions);
        edm_buffer_addrs_per_link.at(link).reserve(
            all_gather_config.get_num_workers_per_link() * num_full_send_directions);
        ...
        for (uint32_t i = 0; i < num_links; ++i) {
...

            {
                ...
                if (rem_pages != 0) {
                    ...
                    TT_ASSERT(
                        rem_pages_per_worker.at(worker_idx % all_gather_config.get_num_workers_per_link()) <=
                        cb_size_in_pages);
                }
                {  // Logging
            ...
            for (uint32_t b = 0; b < all_gather_config.get_num_workers_per_link(); ++b) {
                receiver_semaphores_base_address.push_back(
                    all_gather_config.get_eth_sems_l1_base_byte_address() + b * all_gather_config.get_semaphore_size());
                link_buffer_receiver_addresses.push_back(
                    all_gather_config.get_eth_buffers_l1_base_byte_address() +
                ...
                if (sender_enabled) {
                    ...
                    EriscDatamoverBuilder::ChannelBufferInterface const& sender_channel_buffer_info =
                        sender_edm_builder.add_sender_channel(
                            sender_worker_writer_semaphore_id,
                            clockwise_link_buffer_num_messages_to_send.at(b),
                    ...
                    if (is_channel_shrinkable.at(b) && largest_packets_per_channel.at(b) > 0) {
                        ...
                        log_trace(
                            ...
                            largest_packets_per_channel.at(b),
                            b);
                        sender_edm_builder.set_max_message_size_bytes(
                            sender_channel_buffer_info.channel, largest_packets_per_channel.at(b));
                    }
                ...
                if (receiver_enabled) {
                    ...
                    if (is_channel_shrinkable.at(b) && largest_packets_per_channel.at(b) > 0) {
                        ...
                        log_trace(
                            ...
                            largest_packets_per_channel.at(b),
                            b);
                        receiver_edm_builder.set_max_message_size_bytes(
                            receiver_channel_buffer_info.channel, largest_packets_per_channel.at(b));
                    }
            ...
            for (uint32_t b = 0; b < all_gather_config.get_num_workers_per_link(); ++b) {
                ...
                if (receiver_enabled) {
                    TT_ASSERT(
                        !is_linear || ((is_clockwise_direction && (ring_index != 0)) ||
