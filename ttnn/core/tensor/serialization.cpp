...
void dump_multi_device_host_storage(
    ...
    std::vector<HostBuffer> buffers = [&]() {
        const auto range = storage.distributed_buffer().range();
        std::vector<HostBuffer> buffers;
        buffers.reserve(storage.distributed_buffer().shape().mesh_size());
        for (const auto& coord : range) {
            if (auto buffer = storage.distributed_buffer().get_shard(coord); buffer.has_value()) {
    ...
    std::visit(
        tt::stl::overloaded{
            ...
            [&output_file](const MultiDeviceHostStorage& storage) {
                for (const auto& shard : storage.distributed_buffer().range()) {
                    if (auto buffer = storage.distributed_buffer().get_shard(shard)) {
