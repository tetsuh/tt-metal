...
struct TestFabricFixture {
    ...
    void open_devices(tt::tt_metal::FabricConfig fabric_config) {
        ...
        this->devices_map_ = tt::tt_metal::detail : (this->physical_chip_ids_);
    }
...
public:
    ...
    TestPhysCoords get_dst_from_hops(const std::unordered_map<RoutingDirection, uint32_t>& hops) const;
    TestPhysCoords get_dst_from_hops(const RoutingDirection direction, const uint32_t hops) const;

private:
    void validate_hops(const std::unordered_map<RoutingDirection, uint32_t>& hops) const;
...
inline void TestPhysCoords::validate_hops(const std::unordered_map<RoutingDirection, uint32_t>& hops) const {
...
inline TestPhysCoords::TestPhysCoords(std::tuple<uint32_t, uint32_t> mesh_indices) {
...
inline std::unordered_map<RoutingDirection, uint32_t> TestPhysCoords::get_hops_to_dst(
    ...
    int x_dist = dst_coords.x - this->x_;
    int y_dist = dst_coords.y - this->y_;
...
inline TestPhysCoords TestPhysCoords::get_dst_from_hops(
...
public:
    TestPhysicalMesh(ControlPlane* control_plane_ptr, const mesh_id_t mesh_id);
...
private:
    ...
    chip_id_t get_chip_from_coords(const auto& TestPhysCoords coords) const;
    chip_id_t get_chip_from_hops(
        const chip_id_t src_phys_chip_id, const RoutingDirection direction, const uint32_t hops) const;
    chip_id_t get_chip_from_hops(
        const chip_id_t src_phys_chip_id, const std::unordered_map<RoutingDirection, uint32_t>& hops) const;
...
inline chip_id_t TestPhysicalMesh::get_chip_from_coords(const auto& TestPhysCoords target_coords) const {
    auto it =
...
inline chip_id_t TestPhysicalMesh::get_chip_from_hops(
    ...
    const auto& src_chip_coords = this->physical_chip_coords_[src_phys_chip_id];
...
inline TestPhysicalMesh::TestPhysicalMesh(ControlPlane* control_plane_ptr, const mesh_id_t mesh_id) :
    mesh_id_(mesh_id) {
    const auto& mesh_shape = control_plane_ptr_->get_physical_mesh_shape(mesh_id);
    ...
    for (uint32_t i = 0; i < num_rows; i++) {
        for (uint32_t j = 0; j < num_cols; j++) {
            ...
            this->physical_chip_coords_[physical_chip_id] = TestDevicePhysCoords(std::make_tuple(i, j));
            logical_chip_id++;
        }
    }
}
...
inline std::unordered_map<RoutingDirection, uint32_t> TestPhysicalMesh::get_hops_to_chip(
    ...
    const auto& src_coords = this->physical_chip_coords_[src_phys_chip_id];
    const auto& dst_coords = this->physical_chip_coords_[dst_phys_chip_id];
...
private:
    ...
    std::unordered_map<mesh_id_t, TestPhysicalMesh> physical_meshes_;
...
inline void TestPhysicalMeshes::setup_physical_meshes() {
    ...
    for (const auto& mesh_id : user_meshes) {
        this->physical_meshes_.emplace(mesh_id, this->control_plane_ptr_, mesh_id);
    }
}
...
inline std::unordered_map<RoutingDirection, uint32_t> TestPhysicalMeshes::get_hops_to_chip(
    chip_id_t src_phys_chip_id, chip_id_t dst_phys_chip_id) {
    const auto& src_mesh_id = this->control_plane_->get_fabric_node_id_from_physical_chip_id(src_phys_chip_id).mesh_id;
    const auto& dst_mesh_id = this->control_plane_->get_fabric_node_id_from_physical_chip_id(dst_phys_chip_id).mesh_id;
    ...
    return this->physical_meshes_[src_mesh_id].get_hops_to_chip(src_phys_chip_id, dst_phys_chip_id);
}
...
inline std::vector<chip_id_t> TestPhysicalMeshes::get_chips_from_hops(
    ...
    return this->physical_meshes_[src_mesh_id].get_chips_from_hops(src_phys_chip_id, hops, chip_send_type);
}
