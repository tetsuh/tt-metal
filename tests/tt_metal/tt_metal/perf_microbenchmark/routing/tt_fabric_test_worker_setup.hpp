...
struct TestTrafficSenderConfig {
    TestTrafficDataConfig data_config;
    std::vector<chip_id_t> dst_phys_chip_id;
...
inline void TestTrafficConfig::validate() const {
    ...
    if (!this->dst_phys_chip_ids.has_value() && !this->num_hops.has_value()) {
    ...
    } else if (this->dst_phys_chip_ids.has_value() && this->num_hops.has_value()) {
