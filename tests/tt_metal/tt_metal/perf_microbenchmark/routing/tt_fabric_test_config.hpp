...
struct TestFabricSetup {
    ...
    std::optional<std::vector<std::pair<mesh_id_t, chip_id_t>>> logical_devices;
};
...
std::pair<T1, T2> parse_pair(const YAML::Node& yaml_sequence) {
    ...
    return {yaml_sequence[0].as<T1>(), yaml_sequence[1].as<T2>()};
}
...
std::vector<std::pair<mesh_id_t, chip_id_t>> get_logical_devices(
    mesh_id_t mesh_id, const std::vector<chip_id_t>& logical_chip_ids) {
    std::vector<std::pair<mesh_id_t, chip_id_t>> logical_devices;
...
TestFabricSetup parse_test_fabric_setup(const YAML::Node& test_yaml) {
    ...
    if (devices_yaml["sequence"]) {
        ...
        if (devices_yaml["mesh_id"]) {
            ...
            const auto mesh_id = devices_yaml["mesh_id"].as<mesh_id_t>();
            ...
            if (devices_yaml["sequence"].begin()->IsScalar()) {
                ...
                fabric_setup.logical_devices = parse_pair_sequence<mesh_id_t, chip_id_t>(devices_yaml["sequence"]);
            } else {
        ...
        if (devices_yaml["range"].begin()->IsScalar()) {
            ...
            const auto& logical_range = parse_pair_sequence<mesh_id_t, chip_id_t>(devices_yaml["range"]);
